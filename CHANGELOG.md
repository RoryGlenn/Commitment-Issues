...
- `init` now composes install-time repair safely when a project-owned
  ...
  refuse displaced or duplicate repair commands before mutation; `uninstall`
  restores the original project command byte-for-byte and never claims
  complete removal while an ambiguous owned repair remains.
