...
- Pre-commit linting, formatting, related-test discovery, and optional test
  ...
  batches, and Windows keeps dependency junctions on a traversable filesystem
  without requiring privileged symbolic-link creation.
- `init` now composes install-time repair safely when a project-owned
  ...
  generated by older composition, while `init`, `doctor`, and `uninstall`
  refuse heredoc composition plus displaced or duplicate repair commands,
  including repairs inside shell control flow, before mutation; `uninstall`
  restores the original project command byte-for-byte and never claims complete
  removal while an ambiguous owned repair remains.
