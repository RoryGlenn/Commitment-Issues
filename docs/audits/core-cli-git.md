...
| `commit-fix`                        | none                                                                                    | Safely fixes and amends an unpushed commit                                                                              | Binds exact `HEAD`, index, and target bytes; rechecks publication and refuses pushed, dirty, concurrent, emptying, or manual-only cases. Extra arguments are rejected. `test/commit-fix.test.mjs`, `test/cli.test.mjs`                  |
