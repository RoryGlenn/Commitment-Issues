...
| `scripts/commit-fix.mjs`; `scripts/fix-staged.mjs`; `scripts/lib/fix-safety.mjs`; `scripts/fix-staged-js.mjs` | Prove working-tree/ref safety, transform captured bytes, stage exact attributable blobs, and amend                      | Accepted. Snapshot identities include link count and are revalidated before mutation, co-selected aliases to one inode are rejected before tools run, tools receive target content through stdin, hardlinked repository paths are replaced without modifying external aliases, index installation holds Git's lock, and concurrent edit/delete/type/link/index/HEAD/local-ref changes fail closed. Commit amend additionally refuses detached, signed, tagged, or known-remote-retained history and documents that its proof is an offline local snapshot. |
