...
| `scripts/commit-fix.mjs`          | snapshot-bound fix-and-amend flow, ref/signature/attachment/dirty/concurrent/empty refusal, child cleanup | `test/commit-fix.test.mjs`                                                             |
...
| `scripts/lib/staged-tree.mjs`     | immutable index snapshots, raw blob materialization, and isolated future-tree execution                   | `test/staged-tree.test.mjs`, `test/precommit.test.mjs`                                 |
...
exactly once, while the packed npm lifecycle remains in separate parallel
required jobs. The two Ubuntu coverage lanes stay complete and unsharded, so
