...
/**
 * Build the consumer-owned prepare command that repairs clone-local hooks.
 *
 * Production installs intentionally omit a development-only commitment-issues
 * package. Run the exact project-local CLI when its package manifest exists,
 * but leave the lifecycle successful when the package itself is absent. The
 * generated command uses Node only: it never resolves a global binary, invokes
 * a package-manager runner, or falls back to the network.
 *
 * @param {string | null} integrationManager - Optional supported hook manager.
 * @returns {string} Cross-platform package.json script command.
 */
export function prepareRepairCommand(integrationManager = null) {
  if (
    integrationManager !== null &&
    !HOOK_MANAGERS.includes(integrationManager)
  ) {
    throw new TypeError(`Unsupported hook manager: ${integrationManager}`);
  }

  const integrationArg = integrationManager
    ? `,'--integration=${integrationManager}'`
    : "";
  const source =
    `const p=require('node:path'),r=p.resolve('node_modules/${BIN}'),m=p.join(r,'package.json');` +
    "if(require('node:fs').existsSync(m)){const c=p.join(r,'scripts','cli.mjs');" +
    `process.argv=[process.execPath,c,'doctor','--quiet'${integrationArg}];` +
    "import(require('node:url').pathToFileURL(c).href)}";
  return `node -e "${source}"`;
}

/**
 * Enumerate every prepare repair command currently generated by init.
 *
 * @returns {string[]} Standalone and hook-manager integration commands.
 */
export function prepareRepairCommands() {
  return [
    prepareRepairCommand(),
    ...HOOK_MANAGERS.map((manager) => prepareRepairCommand(manager)),
  ];
}
